...
export class AuthView extends LitElement {
    ...
    async handleGoogleSignIn() {
        this.errorMessage = '';
        this.successMessage = '';
        const googleEmail = prompt('Enter your Google email to Sign In with Google OAuth SSO:');
        ...
        try {
            ...
            const data = await res.json();
            if (!data.success) {
                this.errorMessage = data.error || 'Google Sign In failed.';
                return;
            }
            ...
            this.errorMessage = 'Could not connect to Google auth server.';
        } finally {
            this.isLoading = false;
        }
    }
    ...
    render() {
        ...
        return html`
            <div class="auth-card">
                ...
                <button type="button" class="google-btn" @click=${this.handleGoogleSignIn}>
